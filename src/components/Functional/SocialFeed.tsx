...
export const SocialFeed: ComponentConfig<SocialFeedProps> = {
  ...
  fields: {
    platform:              { type: 'select', label: 'Platform', options: [{ label: 'Instagram (grid preview)', value: 'instagram' }, { label: 'Twitter / X (live timeline)', value: 'twitter' }, { label: 'TikTok (video embed)', value: 'tiktok' }] },
  ...
  render({ platform, username, twitterTimelineHeight, tiktokVideoUrl, instagramColumns, instagramCount, headline, showFollowButton, backgroundColor, textColor, accentColor, borderRadius }) {
    const platformLinks: Record<string, string> = { instagram: 'https://instagram.com/', twitter: 'https://twitter.com/', tiktok: 'https://tiktok.com/@' }
    const platformIcons: Record<string, React.ReactNode> = { instagram: <InstagramIcon />, twitter: <TwitterIcon />, tiktok: <span style={{ fontSize: 18 }}>🎵</span> }
          ...
          {(headline || showFollowButton) && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 36, flexWrap: 'wrap', gap: 12 }}>
              ...
              {showFollowButton && username && (
                <a
                  href={`${platformLinks[platform]}${username}`}
                  target="_blank"
