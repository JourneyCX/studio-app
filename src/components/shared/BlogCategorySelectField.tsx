...
// Blog category dropdown used by BlogPostList's categorySlug field. Same
// pattern as CategorySelectField (WooCommerce product categories), but reads
// the tenant's own Store Blog categories instead — falls back to a plain
// text input (rather than blocking the merchant) if the fetch fails.
export function BlogCategorySelectField({ value, onChange, blankLabel }: {
  value: string
  ...
  useEffect(() => {
    let cancelled = false
    stratumApi.getActiveBlogCategories()
      .then(result => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <span style={{ fontSize: 11, color: '#dd6b20' }}>
          ⚠ Couldn't load category list — enter the slug manually (see Store Blog ▸ Categories).
        </span>
      </div>
    )
  }
  ...
  // renamed, or set before any categories existed) — kept selectable rather
  // than silently discarded, so opening the panel doesn't quietly change the value.
